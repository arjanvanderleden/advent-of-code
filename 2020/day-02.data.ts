export interface PasswordEntry { min: number; max: number; char: string; password: string;}
const isNotEmpty = (s: string) => s.trim().length > 0;
const toPasswordObject = (s: string) => {
    const [_, min, max, char, password ] = s.match(/^(\d+)-(\d+) (\S): (\S+)$/);
    return {
        min: parseInt(min,10),
        max: parseInt(max,10),
        char,
